(() => {
  // src/index.js
  var VERSION = "v0.1.69";
  var NS = "__timelineQaNormalizedFeed";
  var STATUS = { Unset: 0, Active: 1, Disabled: 2, Cancelled: 3 };
  var USAGE = { HomeTeam: 1, HomePlayer: 2, AwayTeam: 3, AwayPlayer: 4, H2H: 5 };
  var suffixes = ["KickOff", "StopFirstHalf", "StartSecondHalf", "StopSecondHalf", "StartFirstHalfExtraTime", "StopFirstHalfExtraTime", "StartSecondHalfExtraTime", "StopSecondHalfExtraTime", "StartPenaltyShootout", "MatchFinished", "InjuryTime", "Goal", "OwnGoal", "GoalScorer", "GoalAssist", "YellowCard", "YellowCardPlayer", "YellowRedCard", "YellowRedCardPlayer", "RedCard", "RedCardPlayer", "SubPlayerOn", "SubPlayerOff", "Corner", "PenaltyAwarded", "PenaltyScored", "PenaltyMissed", "VarStarted", "VarReasonGoal", "VarReasonPenalty", "VarReasonCards", "VarReasonPenaltyNotAwarded", "VarReasonRedCardUpgrade", "VarReasonMistakenIdentity", "VarReasonOther", "VarReasonGoalNotAwarded", "VarReasonRefereeDecisionConfirmed", "VarReasonRefereeDecisionCancelled", "VarReasonRedCardGiven", "VarFinished", "CancelGoal", "CancelYellowCard", "CancelRedCard", "CancelYellowRedCard", "CancelCorner", "CancelPenalty", "CancelSubstitution"];
  var CATALOG = Object.freeze(suffixes.map((suffix, offset) => ({ id: 96 + offset, suffix, name: `GameEvent${suffix}`, label: suffix.replace(/([A-Z])/g, " $1").trim() })));
  var byId = new Map(CATALOG.map((entry) => [entry.id, entry]));
  var PLAYER_TYPES = /* @__PURE__ */ new Set([109, 110, 112, 114, 116]);
  var VAR_TYPES = /* @__PURE__ */ new Set([123, 124, 125, 126, 127, 128, 129, 130, 131, 132, 133, 134, 135]);
  var ROOT_CHILDREN = /* @__PURE__ */ new Map([[107, [109, 110]], [108, [109, 110]], [111, [112]], [113, [114]], [115, [116]], [121, [109]], [123, [124, 125, 126, 127, 128, 129, 130, 131, 132, 133, 134]], [135, [124, 125, 126, 127, 128, 129, 130, 131, 132, 133, 134]]]);
  var CHILD_PARENT_TYPES = new Map([...ROOT_CHILDREN].flatMap(([root, children]) => children.map((child) => [child, root])));
  var SUB_TYPES = /* @__PURE__ */ new Set([117, 118]);
  var clone = (value) => value === void 0 ? void 0 : structuredClone(value);
  var error = (code, message) => Object.assign(new Error(message), { code });
  var exactInteger = (value, label) => {
  ...
  var field = (object, ...names) => names.find((name) => object && object[name] !== void 0) ? object[names.find((name) => object && object[name] !== void 0)] : void 0;
  function getState() {
  ...
  function resolveContext() {
    ...
    const players = (event.subParticipants || []).filter((player) => player?.id !== void 0);
  ...
  function makeEnvelope(context, map) {
    ...
    if (board.igps !== void 0) d.igps = clone(board.igps);
  ...
  function formRecipe(raw, context, ledger) {
    ...
    const team = raw.side === "home" ? context.home : raw.side === "away" ? context.away : null;
    const players = context.players;
    const getPlayer = (name) => {
      const id = raw[name];
      const player = players.find((p) => String(p.id) === String(id));
      if (!player) throw error("PLAYER_REQUIRED", `Select a current event player for ${name}.`);
      return player;
    };
    ...
    const rootIdentity = allocate();
    const root = recordInput({ ...raw, participantId: team?.id ?? 0 }, rootIdentity);
    const result = [{ identity: rootIdentity, record: root, role: "root" }];
    if (raw.delivery !== "complete") return result;
    if (type === 107 || type === 108 || type === 121) {
      const scorerIdentity = allocate();
      result.push({ identity: scorerIdentity, record: playerRecord(raw, scorerIdentity, root, 109, getPlayer("player")), role: "scorer" });
      if (raw.assist) {
        const assistIdentity = allocate();
        result.push({ identity: assistIdentity, record: playerRecord(raw, assistIdentity, root, 110, getPlayer("assist")), role: "assist" });
      }
    } else if (ROOT_CHILDREN.has(type) && raw.player) {
      const childType = ROOT_CHILDREN.get(type)[0], childIdentity = allocate();
      result.push({ identity: childIdentity, record: playerRecord(raw, childIdentity, root, childType, getPlayer("player")), role: "child" });
  ...
  function createPanel(app) {
    ...
    root.querySelector("#tq-close").onclick = () => {
      root.hidden = true;
    };
    root.querySelector("#tq-refresh").onclick = () => app.refresh();
    root.querySelector("#tq-clear").onclick = () => app.clear();
    root.querySelector("#tq-expose").onclick = () => {
      const url = new URL(location.href);
      url.searchParams.set("exposeObgState", "true");
      url.searchParams.set("exposeObgRt", "true");
    ...
    root.querySelector("form").onsubmit = (event) => {
      ...
      app.add(Object.fromEntries(new FormData(event.currentTarget)));
    };
  ...
  function bootstrap() {
    ...
    const app = {
      show(message, bad = false) {
        const node = panel?.querySelector("#tq-status");
        if (node) {
          node.textContent = message;
          node.style.color = bad ? "#b00020" : "#075c25";
        }
      },
      refresh() {
        try {
          ...
          this.show(`Ready for live event ${ctx.eventId}. ${Object.keys(ctx.map).length} current incident records preserved.`);
          this.render(ctx);
        } catch (e) {
          this.show(`${e.code || "ERROR"}: ${e.message}`, true);
        }
      },
      render(ctx) {
        const list = panel?.querySelector("#tq-records");
        if (!list || !ctx) return;
        list.replaceChildren();
        for (const owned of ledger.records(ctx.eventId).values()) {
          const row = document.createElement("li"), actions = document.createElement("span");
          row.append(`${owned.record.gameResultTypeName} \xB7 ${owned.record.reference} \xB7 ${Object.entries(STATUS).find(([, value]) => value === owned.record.status)?.[0]} `);
          for (const [label, status] of [["Disable", STATUS.Disabled], ["Cancel", STATUS.Cancelled], ["Activate", STATUS.Active]]) {
            const button = document.createElement("button");
            button.type = "button";
            button.textContent = label;
            button.onclick = () => this.setStatus(owned.key, status);
      ...
      updateForm(ctx) {
        ...
        try {
          ...
          const form = panel.querySelector("form"), data = new FormData(form), type = Number(data.get("type")), child = data.get("delivery") === "child";
          panel.querySelector(".tq-parent").hidden = !child;
          panel.querySelector(".tq-sub").hidden = !SUB_TYPES.has(type);
          const parent = form.elements.parentKey;
          parent.replaceChildren();
          for (const owned of ledger.records(ctx.eventId).values()) if (compatible(owned, type)) {
            const option = document.createElement("option");
            option.value = owned.key;
            option.textContent = `${owned.record.gameResultTypeName} \u2014 ${owned.reference}`;
            parent.append(option);
          }
          for (const name of ["player", "assist", "subOut"]) {
            const select = form.elements[name], prior = select.value;
            select.replaceChildren();
            const blank = document.createElement("option");
            blank.value = "";
            blank.textContent = name === "assist" ? "None" : "Select player";
            select.append(blank);
            for (const player of ctx.players) {
              const option = document.createElement("option");
              option.value = player.id;
              option.textContent = player.label || player.name || String(player.id);
              select.append(option);
            }
            select.value = prior;
          }
      ...
      add(raw) {
        return this.enqueue(async () => {
          try {
            ...
            const ctx = resolveContext();
            const side = raw.side;
            if (VAR_TYPES.has(Number(raw.type)) && raw.feed === "opta" && side === "none") throw error("PROFILE_REQUIRES_SIDE", "Opta VAR requires Home or Away.");
            if (VAR_TYPES.has(Number(raw.type)) && raw.feed === "runningball" && side !== "none") throw error("PROFILE_REQUIRES_H2H", "RunningBall VAR must use No team / H2H.");
            const recipe = formRecipe(raw, ctx, ledger);
            for (const item of recipe) {
              const latest = resolveContext();
              if (latest.eventId !== ctx.eventId || Object.hasOwn(latest.map, item.identity.key)) throw error("NAVIGATION_OR_OWNERSHIP_CONFLICT", "The event changed or an allocated key is no longer safe.");
              const map = clone(latest.map);
              map[item.identity.key] = item.record;
              window.obgRt.injectMessage(makeEnvelope(latest, map));
              ledger.commit(ctx.eventId, { ...item.identity, record: item.record, feed: raw.feed, role: item.role });
            }
            this.show(`Sent ${recipe.length} normalized realtime record(s); verify the native Timeline.`);
            this.render(resolveContext());
          } catch (e) {
            this.show(`${e.code || "ERROR"}: ${e.message}`, true);
          }
        });
      ...
      setStatus(key, status) {
        return this.enqueue(async () => {
          try {
            readiness();
            const ctx = resolveContext(), owned = ledger.records(ctx.eventId).get(key), map = clone(ctx.map);
            if (!owned || map[key]?.reference !== owned.reference) throw error("OWNERSHIP_CONFLICT", "This record is no longer owned at its original key/reference.");
            map[key].status = status;
            window.obgRt.injectMessage(makeEnvelope(ctx, map));
            owned.record.status = status;
            this.show(`Updated ${owned.reference} status.`);
            this.render(ctx);
          } catch (e) {
            this.show(`${e.code || "ERROR"}: ${e.message}`, true);
          }
      ...
      clear() {
        return this.enqueue(async () => {
          try {
            readiness();
            const ctx = resolveContext(), map = clone(ctx.map);
            let changed = 0;
            for (const owned of ledger.records(ctx.eventId).values()) {
              const current = map[owned.key];
              if (current?.reference !== owned.reference) throw error("OWNERSHIP_CONFLICT", "An owned record was replaced externally; Clear stopped without changing it.");
              current.status = STATUS.Cancelled;
              owned.record.status = STATUS.Cancelled;
              changed++;
            }
            if (changed) window.obgRt.injectMessage(makeEnvelope(ctx, map));
            this.show(changed ? `Cancelled ${changed} owned records.` : "No owned records exist for this event.");
            this.render(ctx);
          } catch (e) {
            this.show(`${e.code || "ERROR"}: ${e.message}`, true);
          }
      ...
      async demo() {
        ...
        panel.querySelector("#tq-demo").disabled = true;
        panel.querySelector("#tq-stop").disabled = false;
        const steps = [[96, "none"], [107, "home"], [111, "away"], [119, "home"], [123, "home"], [121, "away"], [117, "home"], [106, "none"], [98, "none"], [108, "away"], [105, "none"]];
        try {
          for (let i = 0; i < steps.length; i++) {
            if (demoAbort.signal.aborted) throw error("DEMO_STOPPED", "Demo stopped.");
            await this.add({ feed: "opta", type: String(steps[i][0]), delivery: "parent", period: i < 7 ? "1" : "2", minute: String(i * 5 + 1), second: "0", side: steps[i][1], value: "" });
            await new Promise((resolve) => setTimeout(resolve, 80));
          }
          this.show("Full demo delivery completed.");
        } catch (e) {
          this.show(`${e.code || "ERROR"}: ${e.message}`, true);
        } finally {
          demoAbort = void 0;
          panel.querySelector("#tq-demo").disabled = false;
          panel.querySelector("#tq-stop").disabled = true;
        }
      },
      stopDemo() {
        demoAbort?.abort();
    ...
    app.refresh();
    window[NS] = { version: VERSION, refresh: () => app.refresh(), catalog: CATALOG };
  }
  if (typeof window !== "undefined" && typeof document !== "undefined") {
    if (window[NS]) {
      if (window[NS].version !== VERSION) console.error("Timeline QA: a different version is already loaded; reload before injecting.");
      else window[NS].refresh();
    } else bootstrap();
  }
})();
